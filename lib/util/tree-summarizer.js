...
TreeSummary.prototype = {
    ...
    convertToTree: function (summaryMap, arrayPrefix) {
        ...
        Object.keys(summaryMap).forEach(function (key) {
            var parentPath = path.dirname(key) + SEP;
            while (parentPath !== rootPath) {
                var parent = new Node(parentPath, 'dir');
                if (!folders[parentPath]) {
                    folders[parentPath] = parent;
                }
                parentPath = path.dirname(parentPath) + SEP;
            }
        });
        ...
        Object.keys(folders).forEach(function (key) {
            var parentPath = path.dirname(key) + SEP;
            if (parentPath === rootPath) {
                root.addChild(folders[key]);
            } else {
                while (parentPath !== rootPath) {
                    var parent = folders[parentPath];
                    parent.addChild(folders[key]);
                    parentPath = path.dirname(parentPath) + SEP;
                    key = path.dirname(key) + SEP;
                }
                root.addChild(folders[key]);
            }
        });
    ...
    removeDuplicates: function (root) {
        ...
        root.children.forEach(function (child) {
            if (childrenSet.indexOf(child.fullName) === -1) {
                childrenSet.push(child.fullName);
                children.push(child);
            }
        });
        root.children = children;
        root.children.forEach(function (child) {
            self.removeDuplicates(child);
        });
    }
};
